      ...
      <main className="mx-auto w-full max-w-[860px] px-5 pb-20 pt-8 md:px-9">
        ...
        <header className="fade-up mt-10">
          ...
          <p className="mt-5 max-w-[680px] font-cn text-[14px] leading-[1.8] text-ink-soft">
            7 个独立 AI 作品 —— 前 5 个验证单项 AI 能力，后 2 个把这些能力编成共享 Memory 的 AI 陪伴双生子。
        ...
        <section className="fade-up mt-12 flex flex-col gap-5">
          {projects.map((p, i) => {
            ...
            const card = (
              <article className="group relative overflow-hidden rounded-xl border border-line bg-white p-7 transition-all duration-300 hover:border-lav-200 hover:shadow-soft2 md:p-9">
                <span
                  className="absolute right-5 top-5 h-[6px] w-[6px] rounded-full bg-lav-400"
                  style={{ boxShadow: "0 0 0 3px rgba(149,130,201,.15)" }}
                />
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  {isExternal && (
                    <span className="inline-flex shrink-0 items-center gap-1.5 rounded-full border border-lav-100 bg-lav-50 px-3 py-1 font-mono text-[10.5px] text-lav-500 transition-colors group-hover:bg-lav-100">
                      view ↗
                    </span>
                  )}
                </div>
                ...
                {p.description && (
                  <p className="mt-5 font-cn text-[13.5px] leading-[1.8] text-ink-soft">
                    {p.description}
                  </p>
                )}
                ...
                {highlights.length > 0 && (
                  <ul className="mt-6 flex flex-col gap-4">
                    {highlights.map((h, idx) => (
                      <li
                        key={idx}
                        className="border-l-2 border-lav-200 pl-5 transition-colors group-hover:border-lav-400"
                      >
                        {h.tag && (
                ...
                {tags.length > 0 && (
                  <div className="mt-6 flex flex-wrap gap-1.5 border-t border-dashed border-line pt-5">
                    {tags.map((t, idx) => {
